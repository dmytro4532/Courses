...
public interface ITestAttemptRepository
{
    ...
    Task<IEnumerable<TestAttempt>> GetByTestIdAndUserIdAsync(Guid testId, Guid userId, int pageIndex, int pageSize, string orderBy, string orderDirection, CancellationToken cancellationToken);
    ...
    Task<int> CountByTestIdAndUserIdAsync(Guid testId, Guid userId, CancellationToken cancellationToken);
